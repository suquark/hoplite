...
if [ "$#" -eq 2 ]; then
    ...
    log_dir=$working_dir/log/$(date +"%Y%m%d-%H%M%S")-allgather-$1-$2
    mkdir -p $log_dir

    ($working_dir/allgather_test $my_address $my_address $1 0 $2 2>&1 | tee $log_dir/$my_address.server.log) &
    sleep 2
    ...
    $working_dir/allgather_test $3 $my_address $1 $2 $4 2>&1 | tee $5/$my_address.client.log
fi
