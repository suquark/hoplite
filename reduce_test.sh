...
if [ "$#" -eq 2 ]; then
	...
	for index in ${!slaves[@]}
	do
		ssh -t -t ${slaves[$index]} "$(realpath -s $0) $1 $my_address $2 ${object_ids[$index]} $log_dir" &
	done
else
	# sudo fuser -km /tmp/multicast_plasma
	echo "[Putting Object] redis_address: $2 object_size: $3 objectid: $4 my_address: $my_address"
	$working_dir/reduce_test $2 $my_address c $3 $4 2>&1 | tee $5/$my_address.client.log
fi
