...
int main(int argc, char **argv) {
  if (argc != 2) {
    fprintf(stderr, "Usage: ./reduce num_elements\n");
    exit(1);
  }
  ...
  time -= MPI_Wtime();
  MPI_Reduce(rand_nums, global_nums, num_elements_per_proc, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
  time += MPI_Wtime();

  // Print the result
  if (world_rank == 0) {
    printf("MPI_Reduce duration = %lf\n", time);
  }
