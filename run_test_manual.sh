...
if [ "$#" -eq 4 ]; then
	...
	echo "======== on other machine ========"
        echo "$(realpath -s $0) $test_name $world_size $object_size $n_trials $MY_IPADDR $rank $log_dir"
    done
    ...
    gdb -ex=r --args $test_executable_abspath $redis_address $my_address $world_size $rank $object_size $n_trials # 2>&1 | tee $log_dir/$my_address.client.log
fi
