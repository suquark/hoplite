...
int send_all(int conn_fd, const void *buf, const size_t size) {
  size_t cursor = 0;
  while (cursor < size) {
    int bytes_sent = send(conn_fd, (const uint8_t *)buf + cursor, size - cursor, 0);
    if (bytes_sent < 0) {
      LOG(ERROR) << "Socket send error (" << strerror(errno) << ", code=" << errno << ")";
      if (errno == EAGAIN) {
        continue;
      }
      return bytes_sent;
...
int tcp_connect(const std::string &ip_address, int port, int *conn_fd) {
  ...
  *conn_fd = socket(AF_INET, SOCK_STREAM, 0);
  ...
  int status = connect(*conn_fd, (struct sockaddr *)&push_addr, sizeof(push_addr));
  return status;
}
