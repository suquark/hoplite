...
public:
  ...
  grpc::Status ReduceTo(grpc::ServerContext *context, const ReduceToRequest *request, ReduceToReply *reply) {
  ...
  grpc::Status RedirectReduce(grpc::ServerContext *context, const RedirectReduceRequest *request,
...
bool DistributedObjectStore::InvokeReduceTo(const std::string &remote_address, const ObjectID &reduction_id,
  ...
  (void)pool_.push(
      [this](int id, std::string remote_grpc_address, ReduceToRequest request) {
        ...
        DCHECK(status.ok()) << "[GrpcServer] ReduceTo failed at remote address:" << remote_grpc_address
                            << ", message: " << status.error_message() << ", details = " << status.error_code();
        DCHECK(reply.ok());
      },
...
bool DistributedObjectStore::InvokeRedirectReduce(const std::string &remote_address,
  ...
  (void)pool_.push(
      [this](int id, std::string remote_grpc_address, RedirectReduceRequest request) {
        grpc::ClientContext context;
        RedirectReduceReply reply;
        auto stub = get_stub(remote_grpc_address);
...
void DistributedObjectStore::Reduce(const std::vector<ObjectID> &object_ids, const ObjectID &reduction_id,
  ...
  // starting a thread
  std::thread reduction_thread(&DistributedObjectStore::poll_and_reduce, this, object_ids, reduction_id,
                               num_reduce_objects);
  {
    std::lock_guard<std::mutex> l(reduction_tasks_mutex_);
    DCHECK(reduction_tasks_.find(reduction_id) == reduction_tasks_.end())
        << "Reduction task with " << reduction_id.ToString() << " already exists.";
...
void DistributedObjectStore::poll_and_reduce(const std::vector<ObjectID> object_ids, const ObjectID reduction_id,
                                             ssize_t num_reduce_objects) {
  ...
  ssize_t num_remote_reduce_objects = std::max(num_reduce_objects - (ssize_t)local_object_ids.size(), (ssize_t)0);

  std::shared_ptr<ObjectNotifications> notifications =
      gcs_client_.GetLocationAsync(notification_candidates, reduction_id.Binary(), false);
  ...
  DCHECK(pstatus.ok()) << "Plasma failed to create reduction_id = " << reduction_id.Hex() << " size = " << object_size
                       << ", status = " << pstatus.ToString();
  ...
  LOG(DEBUG) << "grid/pipe boundary object size = " << pow(sqrt(P) - 1, 2) * B * L;

  std::unordered_set<ObjectID> reduced_objects;
  if (sqrt(P) - 1 > sqrt(S / (B * L)) && P > 3.5) {
    // NOTE: for P = 16 (including one local node), S < 7.67 MB
    LOG(DEBUG) << "Grid reduce algorithm is used.";
    reduced_objects = poll_and_reduce_grid_impl(notifications, notification_candidates, local_object_ids, object_size,
                                                buffer, reduction_id, num_reduce_objects);
  } else {
    LOG(DEBUG) << "Pipe reduce algorithm is used.";
    reduced_objects = poll_and_reduce_pipe_impl(notifications, notification_candidates, local_object_ids, object_size,
                                                buffer, reduction_id, num_reduce_objects);
  }
...
std::unordered_set<ObjectID> DistributedObjectStore::poll_and_reduce_pipe_impl(
    const std::shared_ptr<ObjectNotifications> &notifications, const std::vector<ObjectID> &notification_candidates,
    std::vector<ObjectID> &local_object_ids, const int64_t object_size, const std::shared_ptr<Buffer> &buffer,
  ...
  std::unordered_set<ObjectID> remaining_ids(notification_candidates.begin(), notification_candidates.end());

  int node_index = 0;
  ObjectID tail_objectid;
  std::string tail_address;
  if (local_object_ids.size() > num_reduce_objects) {
    local_object_ids = std::vector<ObjectID>(local_object_ids.begin(), local_object_ids.begin() + num_reduce_objects);
  }
  ...
  while (num_ready_objects < num_reduce_objects) {
    std::vector<NotificationMessage> ready_ids = notifications->GetNotifications(true);
    ...
    for (auto &ready_id_message : ready_ids) {
      ...
      DCHECK(address != "") << ready_id.ToString() << " location is not ready, but notification is received!";
      LOG(DEBUG) << "Received notification, address = " << address << ", object_id = " << ready_id.ToString();
      ...
      if (address == my_address_) {
        ...
        if (node_index == 1) {
          ...
          bool reply_ok = InvokeReduceTo(tail_address, reduction_id, {ready_id}, address, false, &tail_objectid);
          DCHECK(reply_ok);
        } else if (node_index > 1) {
          // Send 'ReduceTo' command to the other node in the chain.
          bool reply_ok = InvokeReduceTo(tail_address, reduction_id, {ready_id}, address, false);
          DCHECK(reply_ok);
        }
  ...
  if (node_index == 0) {
    ...
    gcs_client_.WriteLocation(reduction_id, my_address_, true, buffer->Size(), buffer->Data());
  } else if (node_index == 1) {
    // only two nodes, no streaming needed
    reply_ok = InvokeReduceTo(tail_address, reduction_id, local_object_ids, my_address_, true, &tail_objectid);
    DCHECK(reply_ok);
  } else {
    // more than 2 nodes
    reply_ok = InvokeReduceTo(tail_address, reduction_id, local_object_ids, my_address_, true);
    DCHECK(reply_ok);
  }
...
std::unordered_set<ObjectID> DistributedObjectStore::poll_and_reduce_grid_impl(
  ...
  ssize_t remaining_num_objects_to_reduce = num_reduce_objects - num_ready_objects;
  if (lines.size() < rows || remaining_num_objects_to_reduce < rows) {
    ...
    size_t n_records_erased =
        notifications->EraseRecords(std::unordered_set<ObjectID>(local_object_ids.begin(), local_object_ids.end()));
    ...
    return poll_and_reduce_pipe_impl(notifications, remaining_candidates, local_object_ids, object_size, buffer,
                                     reduction_id, num_reduce_objects);
  }

  std::vector<ObjectID> edge;
  ssize_t remaining_size = remaining_ids.size();
  std::vector<ObjectID> remaining_ids_list(remaining_ids.begin(), remaining_ids.end());
  ...
  for (int i = 0; i < rows; i++) {
    ...
    ssize_t num_reduce_objects_chain =
        (remaining_num_objects_to_reduce / rows) + (i < remaining_num_objects_to_reduce % rows) + 1;
    ...
    InvokeRedirectReduce(lines[i].first, redirect_object_ids, line_reduction_id, num_reduce_objects_chain);
  }
  ...
  poll_and_reduce_pipe_impl(new_notifications, edge, local_object_ids, object_size, buffer, reduction_id,
                            edge.size() + local_object_ids.size());
  // FIXME(zhuohan): Get the actual reduced objects on remote nodes and return
  // them back.
  for (int i = 0; i < rows; i++) {
    std::unordered_set<ObjectID> reduced_objects_row = RemoteGetReducedObjects(lines[i].first, edge[i]);
    reduced_objects.insert(reduced_objects_row.begin(), reduced_objects_row.end());
  }
